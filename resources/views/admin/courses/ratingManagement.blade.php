                    ...
                    <table class="w-full table-auto">
                        ...
                        <div class="flex justify-between items-center mb-4">
                            <h4 class="text-xl font-bold text-black dark:text-white">
                                Danh sách đánh giá của khóa học <span
                                    class="text-primary">{{ $data->course->CourseName  }}</span>
                            </h4>
                        ...
                        @foreach ($data as $key => $value)
                            <tr>
                                ...
                                <td class="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11 flex flex-col">
                                    <a href="{{ route('users.profile',['id' => $value->user->id]) }}"
                                ...
                                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    {{  count_reply_rating($value->id) > 0 ? count_reply_rating($value->id) . ' lượt phản hồi' : 'Chưa có phản hồi' }}
                                </td>
                                    <?php
                                    $created = new \DateTime($value->created_at, new \DateTimeZone('UTC'));
                                    $created->setTimezone(new \DateTimeZone('Asia/Ho_Chi_Minh'));
                                    ?>
                                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                ...
                                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <div class="flex items-center space-x-3.5">
                                        <a class="hover:text-primary"
                                           href="{{ route('courses.ratingDetail', ['id' => $value->id]) }}">
                                        ...
                                        <form class="deleteRating"
                                              data-redirect="{{ route('courses.ratingManagement', ['id' => $data->course->id]) }}"
                                              method="POST"
                                              data-url="{{ route('courses.deleteRating', $value->id) }}">
                                            @csrf
